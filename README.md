...
* From there, we created the PostreSQL script to create our tables. This can be found in `new_table.py` (`src` folder). We were initially connecting to a local PostreSQL database using Docker, this as later moved to AWS.

* We were given an example CSV file, `chesterfield1.csv`, replicating the type of data we would be handling. The CSV had a lot of PII, so we created a function where we pass in the CSV as a parameter, and the name, card details and card type columns would be removed. It then returns a list of the extracted data.

* We then cleaned this data replacing empty spaces in the size column with “standard”, and made sure each row of data was the same format: 
...
* We created our bucket in the `lambda-template.yaml` file. We deployed serverless changes through the command line. 

* Our handle function in `lambda_handler` ran our lambda function. When triggered, the lambda would read from the bucket, and find the new CSV file. The lambda would then call `lambda_function.py` .

* We setup a CI/CD pipeline Create a IAM user for our CloudFormation deployments.
the secret keys were stored using GitHub Secrets. 
...
-	Finish at a certain time – cut off time for work is 5pm 
